...
import { paymentsApi, branchesApi, counterpartiesApi } from '@/api/services';
import { useCrud } from '@/composables/useCrud';
import { useAuthStore } from '@/stores/auth';
import type { Payment, Branch, Counterparty } from '@/types';
...
onMounted(async () => {
  [branches.value, counterparties.value] = await Promise.all([branchesApi.list(), counterpartiesApi.list()]);
  await crud.load();
});
...
async function handleSave() {
  ...
  form.value.total_amount = computedTotal.value;
  await crud.save();
}
</script>
  ...
  <div class="space-y-5">
    ...
    <div class="rounded-2xl border border-slate-800 bg-[#0e1320] p-2">
      <DataTable :value="items" :loading="loading" paginator :rows="10" data-key="id">
        ...
        <Column header="Naqd">
          <template #body="{ data }">{{ money(data.cash_amount) }}</template>
        </Column>
        <Column header="Plastik">
          <template #body="{ data }">{{ money(data.plastic_amount) }}</template>
        </Column>
