...
import {
  ...
  regionsApi, districtsApi, vehicleModelsApi, fuelTypesApi, documentTypesApi, branchesApi,
} from '@/api/services';
import { useAuthStore } from '@/stores/auth';
import { extractError } from '@/composables/useCrud';
import type { Region, District, VehicleModel, FuelType, DocumentType, Branch, Counterparty, Vehicle } from '@/types';
...
onMounted(async () => {
  [regions.value, districts.value, models.value, fuelTypes.value, documentTypes.value, branches.value, counterparties.value] =
    await Promise.all([
      regionsApi.list(), districtsApi.list(), vehicleModelsApi.list(),
      fuelTypesApi.list(), documentTypesApi.list(), branchesApi.list(), counterpartiesApi.list(),
    ]);
});
  ...
  try {
    ...
    if (addPayment.value && payTotal.value > 0) {
      await paymentsApi.create({
        ...
        plastic_amount: pay.plastic_amount,
        receipt_type: pay.receipt_type as 'INV' | 'FTK',
        z_report_id: pay.z_report_id || null,
      });
    }
